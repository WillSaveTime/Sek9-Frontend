...
// import { addNetwork, removeNetwork } from 'redux/slices/network';
// import { setAuthClear } from 'redux/slices/auth';
...
const checkAndGetNotifyMessage = (method, url) => {
  ...
  if (!method || method === undefined || !url || url === undefined) {
    return { type: null, message: null }
  }

  for (let i = 0; i < notifyExceptionUrlPrefixs.length; i++) {
    if (url.includes(notifyExceptionUrlPrefixs[i]))
      return { type: null, message: null }
  }
  if (method === 'POST') {
...
export const preCheckUrl = (method, url) => {
  ...
  if (method == 'POST' && url.includes('/generate_pdf/')) {
    type = 'info'
    message =
      "Rendu d'un fichier pdf pour AF Checker. Cela prendra plusieurs minutes. Attendez un peu, s'il vous plaît."
  ...
  if (method == 'POST' && url.includes('/prescription/create/')) {
    type = 'info'
    message =
      "Rendu d'un fichier pdf pour la prescription. Cela prendra plusieurs minutes. Attendez un peu, s'il vous plaît."
    // staticStore && staticStore.dispatch(setNotify({ open: true, type, message }));
    return
  }
  ...
  if (method == 'POST' && url.match(regex)) {
    type = 'info'
    message =
      "Rendu d'un fichier pdf pour facture. Cela prendra plusieurs minutes. Attendez un peu, s'il vous plaît."
    // staticStore && staticStore.dispatch(setNotify({ open: true, type, message }));
  }
}
...
export const query = async function(
  ...
  const token = null //(userInfo && userInfo.token);
